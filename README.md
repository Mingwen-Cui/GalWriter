...
| Web 版 | 访问部署好的网页版本，或自行运行 Web 构建 | 快速体验、在线演示、轻量编辑 |
| Android 版 | 从发布页获取 APK，或按 Android 打包流程自行构建 | 移动端预览、轻量编辑、随身查看项目 |
...
3. 打开 APK 完成安装。
4. 启动 GalWriter AI，创建新项目或导入项目 ZIP。
...
| --- | --- | --- | --- |
| 主要用途 | 正式创作、长期项目、本地交付 | 快速体验、在线演示、轻量编辑 | 移动端预览、随身编辑、轻量项目维护 |
| 打包方式 | Tauri Windows 应用 | Web 前端构建 | Tauri Android 应用 |
| 数据位置 | 本机应用数据和用户选择的导出文件 | 浏览器本地存储 | Android 应用数据和用户选择的导出文件 |
| 文件能力 | 支持更完整的本地文件读写、项目导入导出和素材管理 | 受浏览器权限限制，文件访问需要用户手动选择 | 受 Android 文件权限影响，适合导入导出和轻量素材管理 |
| AI 配置 | 保存在当前设备本地 | 保存在当前浏览器环境中 | 保存在当前 Android 设备本地 |
| 视频导出 | 适合正式导出、较长内容和本地保存 | 适合预览或轻量导出 | 适合移动端预览和轻量导出 |
...
- Web 版、Windows 版和 Android 版共用前端渲染逻辑，尽量保证预览效果与导出结果一致。
- Web 版适合短内容预览和轻量导出，输出能力会受到浏览器编码支持、内存和标签页生命周期影响。
- Windows 版和 Android 版适合在本地应用环境中保存导出结果；其中 Windows 版更适合较长内容和正式交付。
...
├── public/                      # Web 静态资源和托管 API 示例
│   └── api/                     # Web 托管代理相关文件
├── docs/                        # 项目文档与发布说明
├── landingpage/                 # 独立官网/落地页素材
├── release/                     # 本地打包输出目录
├── dist/                        # Web 构建输出目录
...
不同浏览器对视频编码和封装格式的支持并不完全一致。GalWriter AI 会优先使用当前运行环境中稳定可用的导出路径；如果某个格式在当前环境不可用，建议改用桌面版或选择应用提供的可用格式。

</details>

<details>
<summary><strong>我需要自己安装额外的视频工具吗？</strong></summary>

当前 README 不再把外部视频工具作为导出的前置要求。一般情况下，按应用内的视频导出流程操作即可。
