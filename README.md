...
### 面向视觉小说、Galgame 与分支互动叙事的本地优先 AI 创作工作台
...
- **可视化故事画布**：支持 `storyNode`、`characterNode`、`sceneNode`、`aiNode`、`backgroundNode`、`groupNode`、`numberConditionNode`、`batchReplaceNode`、`plotStructureNode`、`textNode`、`summaryNode`。
...
| 平台 | 当前定位 | 说明 |
| --- | --- | --- |
| Windows | 主力创作与正式导出 | Tauri 原生保存对话框、本地导出路径、系统 TTS、视频与 Web 导出保存流程更完整 |
| Web | 快速体验与轻量创作 | 使用浏览器本地存储，适合快速试用、轻量编辑、短内容预览与导出 |
| Android | 移动端预览与测试 | 使用 Tauri Android 打包，更偏向移动端查看、试跑与轻量交互 |
