...
| Web 版 | ホスト済み Web 版を利用、または自分で Web ビルドを実行 | クイック試用、オンラインデモ、軽量編集 |
| Android 版 | リリースページから APK を入手、または Android パッケージング手順で自分でビルド | モバイルプレビュー、軽量編集、外出先でのプロジェクト確認 |
...
4. GalWriter AI を起動し、新規プロジェクトを作成するか、プロジェクト ZIP をインポートします。
...
| --- | --- | --- | --- |
| 主な用途 | 本格制作、長期プロジェクト、ローカル納品 | クイック試用、オンラインデモ、軽量編集 | モバイルプレビュー、外出先での編集、軽量なプロジェクト管理 |
...
| ファイル機能 | ローカルファイル操作、プロジェクト入出力、素材管理をより広く扱える | ブラウザ権限に制限され、ファイルはユーザー選択が必要 | Android のファイル権限に影響され、入出力と軽量素材管理に向く |
| AI 設定 | 現在の端末に保存 | 現在のブラウザ環境に保存 | 現在の Android 端末に保存 |
| 動画書き出し | 正式な書き出し、長い内容、ローカル保存に向く | プレビューや軽量な書き出しに向く | モバイルプレビューと軽量な書き出しに向く |
...
- Web 版、Windows 版、Android 版は同じフロントエンド描画ロジックを共有し、プレビューと書き出し結果ができるだけ一致するようにします。
- Web 版は短い内容のプレビューと軽量な書き出しに向いています。出力能力はブラウザのエンコード対応、メモリ、タブのライフサイクルに影響されます。
- Windows 版と Android 版はネイティブアプリ環境で保存できます。特に Windows 版は長い内容と正式な納品に向いています。
...
├── public/                      # Web 静的リソースとホスト API 例
│   └── api/                     # Web ホストプロキシ関連ファイル
├── docs/                        # プロジェクト文書とリリースノート
├── landingpage/                 # 独立公式サイト / ランディングページ素材
...
ブラウザごとに動画エンコードとコンテナ形式の対応は異なります。GalWriter AI は現在の実行環境で安定して利用できる書き出し経路を優先します。特定の形式が現在の環境で利用できない場合は、Windows 版を使うか、アプリが提示する利用可能な形式を選んでください。

</details>

<details>
<summary><strong>追加の動画ツールを自分でインストールする必要がありますか？</strong></summary>

現在の README では、外部動画ツールを動画書き出しの前提条件としていません。通常はアプリ内の動画書き出しフローをそのまま利用できます。
