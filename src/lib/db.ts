...
const DEFAULT_AI_PROFILES_STATE: LocalAIProfilesState = {
  ...
  activeBackgroundRemovalProfileId: null,
...
const isAutogeneratedProfile = (profile: SavedAIProfile) => {
  ...
  if (profile.kind === 'background-removal') return false;
...
const normalizeAIProfilesState = (
  ...
  const getActiveId = (kind: SavedAIProfile['kind'], incoming: string | null | undefined) => {
  ...
  return {
    ...
    activeBackgroundRemovalProfileId: getActiveId(
      'background-removal',
      state?.activeBackgroundRemovalProfileId,
    ),
