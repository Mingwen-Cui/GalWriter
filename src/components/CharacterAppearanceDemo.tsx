  ...
  return <div className="fixed inset-0 z-[1100] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm" onMouseDown={onClose}>
    <section className="grid max-h-[min(760px,calc(100vh-32px))] w-full max-w-4xl overflow-hidden rounded-2xl border border-white/60 bg-[var(--card-bg)] shadow-2xl md:grid-cols-[minmax(0,1fr)_320px]" role="dialog" aria-modal="true" aria-label={labels.title} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="overflow-y-auto p-4 custom-scrollbar"><div className="mb-4 flex items-center justify-between"><div><h2 className="text-sm font-bold text-[var(--text-primary)]">{labels.title}</h2><p className="mt-0.5 text-[10px] text-[var(--text-muted)]">{faces.length} \u4e2a\u8111\u888b \u00b7 {hairs.length} \u5bf9\u53d1\u578b \u00b7 {outfits.length} \u4ef6\u8863\u670d</p></div><button type="button" onClick={onClose} className="rounded-lg px-2 py-1 text-sm text-[var(--text-muted)] hover:bg-[var(--app-bg)]">\u00d7</button></div>
        ...
        <div className="mt-4 space-y-2"><label className="text-[11px] font-bold text-[var(--text-secondary)]">{labels.head}</label><div className="flex flex-wrap gap-2">{faces.map(({ id, label }) => <OptionButton key={id} active={appearance.faceId === id} onClick={() => setAppearance((current) => ({ ...current, faceId: id }))}>{label}</OptionButton>)}</div></div>
