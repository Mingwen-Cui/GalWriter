...
export function SceneCanvasInspector({ language, value, onChange, mode = 'position' }: Props) {
  ...
  return (
    <div className="space-y-3 text-xs text-slate-900 dark:text-[var(--vr-text)]">
      {mode === 'position' && <section className="property-section">
        <div className="mb-3">
      ...
      {mode === 'background' && <BackgroundFillInspector language={language} allowedTypes={['solid','gradient','image']} gradientShapes={false} value={{
        type: value.sceneBackgroundType, color: value.sceneBackgroundColor, gradientStart: value.sceneBackgroundGradientStart, gradientEnd: value.sceneBackgroundGradientEnd, gradientAngle: value.sceneBackgroundGradientAngle, gradientStops: value.sceneBackgroundGradientStops, imageUrl: value.sceneBackgroundImageUrl,
      }} onChange={patch => {
        ...
        if (patch.gradientAngle !== undefined) mapped.sceneBackgroundGradientAngle = patch.gradientAngle;
        if (patch.gradientStart !== undefined) mapped.sceneBackgroundGradientStart = patch.gradientStart;
        if (patch.gradientEnd !== undefined) mapped.sceneBackgroundGradientEnd = patch.gradientEnd;
        if (patch.gradientStops !== undefined) mapped.sceneBackgroundGradientStops = patch.gradientStops;
        if (patch.imageUrl !== undefined) mapped.sceneBackgroundImageUrl = patch.imageUrl;
        onChange({...mapped, sceneBackgroundVisible: true});
      }}/>}
    </div>
  );
}
