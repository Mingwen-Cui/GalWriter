...
export function resizeCanvas(
  ...
  field: 'canvasWidth' | 'canvasHeight',
  next: number,
  ...
  const normalized = normalizeSharedCanvasSettings({
    ...value,
    canvasWidth: width,
    canvasHeight: height,
  });
  return {
    ...
    ...canvasRatio(normalized.canvasWidth, normalized.canvasHeight),
  };
}

export function migrateVideoCanvasSettings(
  previous?: {
  ...
  if (previous?.videoCanvasSettings)
    return normalizeSharedCanvasSettings(previous.videoCanvasSettings);
  const canvasWidth = previous?.resolutionWidth || 1920;
  const canvasHeight = previous?.resolutionHeight || 1080;
  return normalizeSharedCanvasSettings({
    ...previous?.webSettings,
    canvasWidth,
    canvasHeight,
