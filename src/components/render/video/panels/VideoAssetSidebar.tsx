...
type VideoAssetSidebarProps = {
  ...
  openContextMenu: (event: React.MouseEvent<HTMLElement>, target: { kind: 'asset'; nodeId: string }) => void;
  addNodeToTimeline: (
    id: string,
  ...
  handleAssetScaleHandleStart: (event: React.PointerEvent<HTMLButtonElement>, side: 'top' | 'bottom') => void;
                    ...
                    >
                      ...
                      {visibleAssetNodes.length === 0 && (
                        <div className="rounded-lg border border-dashed border-[var(--vr-border-strong)] px-3 py-8 text-center text-xs font-bold text-[var(--vr-text-muted)]">
                          {t('这个区域里暂无可渲染卡片', 'この領域に書き出せるカードはありません', 'No renderable cards in this region')}
                        </div>
                      )}
                    </div>
                  </div>
                  <div className="absolute inset-y-3 right-2 w-4">
                    <div className="relative h-full w-2 rounded-full bg-slate-200">
                      ...
                      <div
                        ...
                        title={t('拖动滚动素材卡片', 'ドラッグして素材カードをスクロール', 'Drag to scroll asset cards')}
                      >
                        ...
                        <button
                          ...
                          title={t('拖动调整素材卡片缩放上手柄', 'ドラッグして素材カード倍率の上ハンドルを調整', 'Drag to adjust asset scale top handle')}
                          aria-label={
                            t('调整素材卡片缩放上手柄', '素材カード倍率の上ハンドルを調整', 'Adjust asset scale top handle')
                          }
                        />
                        <button
                          type="button"
                          ...
                          onPointerUp={handleAssetScaleHandleEnd}
                          onPointerCancel={handleAssetScaleHandleEnd}
                          title={t('拖动调整素材卡片缩放下手柄', 'ドラッグして素材カード倍率の下ハンドルを調整', 'Drag to adjust asset scale bottom handle')}
                          aria-label={
                            t('调整素材卡片缩放下手柄', '素材カード倍率の下ハンドルを調整', 'Adjust asset scale bottom handle')
                          }
                        />
                      </div>
                    </div>
