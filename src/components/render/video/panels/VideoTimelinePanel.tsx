...
import type { RenderStatus, TimelineScaleMode, TimelineSegmentMetric, TimelineWheelMode } from '../shared/types';
...
type VideoTimelinePanelProps = {
  ...
  timelineMetrics: { segments: TimelineSegmentMetric[]; totalDuration: number; displayDuration: number; width: number; pixelsPerSecond: number };
  ...
  seekTimelineTime: (time: number, options?: { keepPlaying?: boolean; preserveFocus?: boolean }) => void;
  ...
  handleTimelineScaleHandleStart: (event: React.PointerEvent<HTMLButtonElement>, side: 'left' | 'right') => void;
  ...
  return (
            ...
            >
              <div className="absolute inset-x-0 top-0">
                <ResizeHandle
                  label={t('调整视频编辑时间线高度', '編集タイムラインの高さを調整', 'Resize editing timeline')}
                  axis="y"
                  ...
                  <div className="flex h-8 rounded-lg bg-[var(--vr-surface-soft)] p-0.5">
                    {(['vertical', 'horizontal'] as TimelineWheelMode[]).map((mode) => (
                      <button
                        ...
                        title={
                          mode === 'vertical'
                            ? t('鼠标滚轮上下滚动时间线区域', 'ホイールで縦スクロール', 'Mouse wheel scrolls vertically')
                            : t('鼠标滚轮左右移动时间线', 'ホイールで横スクロール', 'Mouse wheel scrolls horizontally')
                        }
                      >
                        {mode === 'vertical' ? (
                ...
                <div className="min-w-max space-y-3">
                  ...
                  >
                    ...
                    <div
                      ...
                      title={t('拖动或点击移动播放条', 'ドラッグまたはクリックで再生位置を移動', 'Drag or click to move playhead')}
                    >
                    ...
                    <div className="space-y-0">
                            ...
                            >
                                    ...
                                    <div className="relative z-10 flex items-center justify-end">
                                      <div className="flex items-center gap-1">
                                        <button
                                          ...
                                          title={t('从时间线删除', 'タイムラインから削除', 'Remove from timeline')}
                                        >
                              ...
                              {trackNodes.length === 0 ? (
                                <div
                                  ...
                                  style={{ width: timelineMetrics.width }}
                                >
                                  {t('把左侧素材拖到这里', '左側の素材をここへドラッグ', 'Drag assets here')}
                                </div>
                              ) : null}
                            </div>
                          </div>
                      ...
                      <div className="relative h-4">
                        ...
                        <div
                          ...
                          onPointerUp={handleTimelineScrubEnd}
                          onPointerCancel={handleTimelineScrubEnd}
                          title={t('拖动调整播放条', 'ドラッグして再生位置を調整', 'Drag to adjust playhead')}
                        >
                            ...
                            >
                              ...
                              {trackNodes.length === 0 ? (
                                <div
                                  ...
                                  style={{ width: timelineMetrics.width }}
                                >
                                  {t('把音频片段拖到这里', '音声クリップをここへドラッグ', 'Drag audio clips here')}
                                </div>
                              ) : null}
                            </div>
                          </div>
                ...
                <div className="flex h-full w-full items-center">
                  <div className="relative h-2 w-full rounded-full bg-slate-200">
                    ...
                    <div
                      ...
                      title={t('拖动移动时间线视野', 'ドラッグしてタイムライン表示を移動', 'Drag to scroll timeline')}
                    >
                      ...
                      <button
                        ...
                        title={t('拖动调整可视窗口宽度', 'ドラッグして表示範囲の幅を調整', 'Drag to resize timeline window')}
                        aria-label={t('调整时间轴缩放左手柄', 'タイムライン倍率の左ハンドルを調整', 'Adjust timeline scale left handle')}
                      />
                      <button
                        type="button"
                        ...
                        title={t('拖动调整可视窗口宽度', 'ドラッグして表示範囲の幅を調整', 'Drag to resize timeline window')}
                        aria-label={
                          t('调整时间轴缩放右手柄', 'タイムライン倍率の右ハンドルを調整', 'Adjust timeline scale right handle')
                        }
                      />
                    </div>
                  </div>
