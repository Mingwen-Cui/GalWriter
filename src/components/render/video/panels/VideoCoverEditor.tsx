...
import { Check, Download, ImagePlus, Redo2, Save, Settings2, Trash2, Type, Undo2, Upload, X, ZoomOut } from 'lucide-react';
...
const cloneCoverSettings = (value: VideoCoverSettings) => JSON.parse(JSON.stringify(value)) as VideoCoverSettings;
...
const coverTextShadowStyle = (element: VideoCoverElement, stageHeight: number, canvasHeight: number) => {
  ...
  return shadows
    ...
    .map((shadow) => `${(shadow.offsetX || 0) * scale}px ${(shadow.offsetY || 0) * scale}px ${(shadow.blur || 0) * scale}px ${textColorWithAlpha(shadow.color, shadow.opacity)}`)
    .join(', ') || 'none';
};
...
export function VideoCoverEditor({ cover, nodes, resolution, language, onChange, onDelete, onClose, embedded = false }: VideoCoverEditorProps) {
  ...
  const coverHistoryRef = useRef<{ past: VideoCoverSettings[]; future: VideoCoverSettings[] }>({ past: [], future: [] });
  ...
  const templateImages = useMemo(
    () => templateLibrary.flatMap((template) => template.assets.map((asset) => ({
    ...
    const loadTemplateLibrary = async () => {
      try {
        ...
        if (isTauriRuntime()) {
          ...
          if (response.ok) {
            templates = ((await response.json()) as { templates?: CoverTemplateInfo[] }).templates || [];
          }
        }
  ...
  useEffect(() => {
    ...
    legacy.push({ id: 'cover-logo', kind: 'image', imageUrl: '/glass.png', visible: true, x: 92, y: 88, width: 5, height: 8.8, rotation: 0, opacity: 94, objectFit: 'contain', zIndex: 100 });
  ...
  const patch = (value: Partial<VideoCoverSettings>, recordHistory = true) => commitCover({ ...cover, ...value }, recordHistory);
  ...
  const updateElements = (next: VideoCoverElement[], recordHistory = true) => patch({ elements: next }, recordHistory);
  const updateElement = (id: string, value: Partial<VideoCoverElement>, recordHistory = true) => updateElements(elements.map((element) => element.id === id ? { ...element, ...value } : element), recordHistory);
  ...
  const addText = () => {
    const element: VideoCoverElement = { id: newElementId('text'), kind: 'text', text: language === 'zh' ? '输入文字' : language === 'ja' ? 'テキスト' : 'Your text', visible: true, x: 20, y: 42, width: 60, height: 13, rotation: 0, opacity: 100, fontSize: 58, fontWeight: 800, textAlign: 'center', textColor: '#ffffff', zIndex: Math.max(0, ...elements.map((item) => item.zIndex ?? 0)) + 1 };
    updateElements([...elements, element]); setSelectedElementId(element.id);
  };
  const addImage = (imageUrl: string) => {
    if (!imageUrl) return;
    const element: VideoCoverElement = { id: newElementId('image'), kind: 'image', imageUrl, visible: true, x: 32, y: 32, width: 36, height: 36, rotation: 0, opacity: 100, objectFit: 'cover', zIndex: Math.max(0, ...elements.map((item) => item.zIndex ?? 0)) + 1 };
    updateElements([...elements, element]); setSelectedElementId(element.id);
  ...
  const applyTemplate = async (template: CoverTemplateInfo) => {
    ...
    const assetUrls = template.assets.map((asset) => encodeURI(`/cover-templates/${template.id}/${asset}`));
    ...
    if (savedSettings) {
      ...
      imageLayers.push({
        ...
        x: 92, y: isPortrait ? 94 : 88, width: isPortrait ? 6 : 5, height: isPortrait ? 4 : 8.8,
        rotation: 0, opacity: 94, objectFit: 'contain', zIndex: 100,
      });
      commitCover({
        ...cover,
        sourceType: 'gradient',
  ...
  const saveActiveTemplate = async () => {
    ...
    try {
      ...
      await invoke('save_cover_copy', { templateId: activeTemplateId, settings: cover, previewPng });
  ...
  const beginDrag = (event: ReactPointerEvent<HTMLDivElement>, element: VideoCoverElement) => {
    ...
    recordCoverHistory();
    const move = (moveEvent: PointerEvent) => updateElement(element.id, { x: clamp(element.x + ((moveEvent.clientX - startX) / rect.width) * 100, -100, 100), y: clamp(element.y + ((moveEvent.clientY - startY) / rect.height) * 100, -100, 100) }, false);
  ...
  const beginResize = (event: ReactPointerEvent<HTMLElement>, element: VideoCoverElement, handle: WebEditableResizeHandle) => {
    ...
    const move = (moveEvent: PointerEvent) => {
      ...
      if (handle.includes('w')) { x = clamp(element.x + dx, -100, 100); width = clamp(element.width - (x - element.x), 4, 200); }
      if (handle.includes('n')) { y = clamp(element.y + dy, -100, 100); height = clamp(element.height - (y - element.y), 4, 200); }
  ...
  const beginRotate = (event: ReactPointerEvent<HTMLElement>, element: VideoCoverElement) => {
    ...
    recordCoverHistory();
    const move = (moveEvent: PointerEvent) => { const angle = Math.atan2(moveEvent.clientY - centerY, moveEvent.clientX - centerX); updateElement(element.id, { rotation: element.rotation + ((angle - startAngle) * 180) / Math.PI }, false); };
    const end = () => { window.removeEventListener('pointermove', move); window.removeEventListener('pointerup', end); };
    window.addEventListener('pointermove', move); window.addEventListener('pointerup', end);
  };
  ...
  return <div className={embedded ? 'h-full min-h-0 min-w-0 overflow-hidden bg-[var(--vr-bg)]' : 'fixed inset-0 z-[200] grid place-items-center bg-slate-950/20 p-6'}>
    <div className={`relative grid w-full overflow-hidden bg-[var(--vr-bg)] grid-rows-[52px_minmax(0,1fr)] ${embedded ? 'h-full' : 'h-[min(760px,calc(100vh-48px))] max-w-[1180px] rounded-2xl border border-white/70 shadow-2xl'}`}>
      <header className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 border-b border-[var(--vr-border)] bg-[var(--vr-surface)] px-4">
        ...
        <div className="flex justify-end gap-2">{activeTemplateId && <><button type="button" onClick={() => void exportActiveTemplate()} disabled={isTemplateExporting} className="flex h-9 items-center gap-1.5 rounded-lg border border-violet-200 bg-violet-50 px-3 text-xs font-black text-violet-700 hover:bg-violet-100 disabled:opacity-50"><Download className="h-4 w-4" />{isTemplateExporting ? '导出中' : '导出模板'}</button>{isTauriRuntime() ? <button type="button" onClick={() => void saveActiveTemplate()} disabled={templateSaveState === 'saving'} className="flex h-9 items-center gap-1.5 rounded-lg border border-emerald-200 bg-emerald-50 px-3 text-xs font-black text-emerald-700 hover:bg-emerald-100 disabled:opacity-50"><Save className="h-4 w-4" />{templateSaveState === 'saving' ? '保存中' : templateSaveState === 'saved' ? '已保存副本' : templateSaveState === 'error' ? '保存失败，重试' : '保存副本'}</button> : <button type="button" onClick={() => void downloadCoverPng()} className="flex h-9 items-center gap-1.5 rounded-lg border border-sky-200 bg-sky-50 px-3 text-xs font-black text-sky-700 hover:bg-sky-100"><Download className="h-4 w-4" />下载封面</button>}</>}<button type="button" onClick={() => setViewScale((current) => current <= 0.5 ? 1 : Math.max(0.5, current - 0.2))} className="flex h-9 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 text-xs font-black text-slate-600 hover:bg-slate-50" title="缩小画布视角，查看超出画布的图层"><ZoomOut className="h-4 w-4" />{viewScale <= 0.5 ? '还原视角' : '缩小视角'}</button><button type="button" onClick={() => void closeEditor()} className="flex h-9 items-center gap-1.5 rounded-lg bg-[var(--vr-accent)] px-3 text-xs font-black text-white hover:brightness-105"><Check className="h-4 w-4" />{text.done}</button><button type="button" onClick={() => void closeEditor()} className="grid h-9 w-9 place-items-center rounded-lg bg-[var(--vr-surface-soft)] text-[var(--vr-text-soft)] hover:bg-[var(--vr-border)]" aria-label="Close"><X className="h-4 w-4" /></button></div>
      </header>
      {isImagePickerOpen && <div className="absolute left-1/2 top-14 z-[300] w-[min(520px,calc(100%-32px))] -translate-x-1/2 rounded-2xl border border-[var(--vr-border)] bg-[var(--vr-surface)] p-4 shadow-2xl">
        ...
        <div className="grid max-h-72 grid-cols-4 gap-2 overflow-y-auto">{templateImages.map((image) => <button key={image.id} type="button" onClick={() => addImage(image.url)} className="aspect-square overflow-hidden rounded-xl border border-indigo-100 bg-indigo-50 hover:ring-2 hover:ring-[var(--vr-accent)]" title={image.label}><img src={image.url} alt="" className="h-full w-full object-cover" /></button>)}{imageNodes.map((node) => { const url = String(node.data?.imageUrl || ''); return <button key={node.id} type="button" onClick={() => addImage(url)} className="aspect-square overflow-hidden rounded-xl border border-[var(--vr-border)] bg-[var(--vr-surface-soft)] hover:ring-2 hover:ring-[var(--vr-accent)]" title={nodeLabel(node)}><img src={url} alt="" className="h-full w-full object-cover" /></button>; })}</div>
        {!imageNodes.length && !templateImages.length && <p className="py-5 text-center text-xs text-[var(--vr-text-muted)]">{text.noMedia}</p>}
      </div>}
      <div className="grid min-h-0 grid-cols-[minmax(0,1fr)_330px] max-md:grid-cols-1">
        <section className="flex min-h-0 min-w-0 flex-col bg-[var(--vr-surface-soft)]"><div className="min-h-0 flex-1 overflow-hidden p-5" onPointerDown={() => { setSelectedElementId(null); setEditingElementId(null); }}><div className="mx-auto flex h-full min-h-[360px] items-center justify-center"><div ref={stageRef} className="relative w-full max-w-[1120px] overflow-visible bg-slate-950 shadow-2xl transition-transform" style={{ aspectRatio: `${coverCanvasSettings.canvasWidth} / ${coverCanvasSettings.canvasHeight}`, transform: `scale(${viewScale})` }} onPointerDown={() => { setSelectedElementId(null); setEditingElementId(null); }}>
          <canvas ref={previewRef} className="absolute inset-0 h-full w-full" />
          {orderedElements.map((element) => <div key={element.id} data-cover-element="true" className={`absolute cursor-move touch-none ${element.visible ? '' : 'opacity-35'}`} style={{ left: `${element.x}%`, top: `${element.y}%`, width: `${element.width}%`, height: `${element.height}%`, transform: `rotate(${element.rotation}deg)`, opacity: element.opacity / 100, zIndex: element.zIndex ?? 0 }} onPointerDown={(event) => beginDrag(event, element)}>
            {element.kind === 'text' ? <div contentEditable={editingElementId === element.id} suppressContentEditableWarning onDoubleClick={(event) => { event.stopPropagation(); setSelectedElementId(element.id); setEditingElementId(element.id); event.currentTarget.focus(); }} onBlur={(event) => { updateElement(element.id, { text: event.currentTarget.innerText }); setEditingElementId(null); }} className="flex h-full w-full items-center overflow-hidden whitespace-pre-wrap break-words px-1 leading-tight outline-none" style={{ color: textColorWithAlpha(element.textColor, element.textColorAlpha), fontFamily: element.fontFamily, fontSize: `${Math.max(12, ((element.fontSize || 54) / coverCanvasSettings.canvasHeight) * stageHeight)}px`, fontWeight: element.fontWeight || 800, letterSpacing: `${((element.letterSpacing || 0) / coverCanvasSettings.canvasHeight) * stageHeight}px`, lineHeight: element.lineHeight || 1.28, textAlign: element.textAlign || 'center', justifyContent: element.textAlign === 'left' ? 'flex-start' : element.textAlign === 'right' ? 'flex-end' : 'center', textShadow: coverTextShadowStyle(element, stageHeight, coverCanvasSettings.canvasHeight), WebkitTextStroke: element.strokeEnabled === false || !(element.textStrokeWidth || 0) ? undefined : `${((element.textStrokeWidth || 0) / coverCanvasSettings.canvasHeight) * stageHeight}px ${element.textStrokeColor || '#000000'}`, paintOrder: 'stroke fill' }}>{element.text}</div> : element.imageUrl ? <div className="h-full w-full overflow-hidden"><img src={element.imageUrl} alt="" draggable={false} className="h-full w-full select-none" style={{ objectFit: element.objectFit || 'cover', objectPosition: `${element.imageCropX ?? 50}% ${element.imageCropY ?? 50}%`, transform: `scale(${element.imageCropScale ?? 1})`, transformOrigin: `${element.imageCropX ?? 50}% ${element.imageCropY ?? 50}%` }} /></div> : null}
        ...
        </div></div></div><div className="border-t border-[var(--vr-border)] bg-[var(--vr-surface)] p-3"><div className="grid grid-cols-4 gap-2">{templateLibrary.slice(0, 4).map((template) => { const previewUrl = encodeURI(`/cover-templates/${template.id}/preview.png?${templatePreviewVersion}`); const firstAssetUrl = template.assets[0] ? encodeURI(`/cover-templates/${template.id}/${template.assets[0]}`) : ''; return <button key={template.id} type="button" title={template.id} aria-label={template.id} onClick={() => void applyTemplate(template)} className={`group overflow-hidden rounded-xl border p-1 transition-all ${activeTemplateId === template.id ? 'border-[var(--vr-accent)] ring-2 ring-[var(--vr-accent-soft)]' : 'border-[var(--vr-border)] hover:border-[var(--vr-accent)]'}`}><span className="relative flex h-14 items-center justify-center overflow-hidden rounded-lg bg-slate-800">{firstAssetUrl && <img src={previewUrl} alt="" className="h-full w-full object-cover" onError={(event) => { event.currentTarget.src = firstAssetUrl; }} />}</span></button>; })}{isTemplateLibraryLoading && <span className="col-span-4 py-4 text-center text-xs font-bold text-[var(--vr-text-muted)]">读取模板素材…</span>}{!isTemplateLibraryLoading && !templateLibrary.length && <span className="col-span-4 py-4 text-center text-xs font-bold text-[var(--vr-text-muted)]">暂无可用封面模板。</span>}</div></div></section>
        <aside className="min-h-0 overflow-y-auto border-l border-[var(--vr-border)] bg-[var(--vr-surface)] p-4 max-md:border-l-0 max-md:border-t">
          ...
          {selectedElement && selectedInspectorElement ? <><StartMenuElementInspector element={selectedInspectorElement} language={language} showDescriptions={false} fontSizeMax={Number.MAX_SAFE_INTEGER} onUpdate={(updates) => updateElement(selectedElement.id, updates as Partial<VideoCoverElement>)} onAlignSelected={(axis, value) => { const next = value === 'start' ? 0 : value === 'center' ? 50 : 100; updateElement(selectedElement.id, axis === 'x' ? { x: next - selectedElement.width / 2 } : { y: next - selectedElement.height / 2 }); }} />{selectedElement.kind === 'image' && <section className="mt-4 rounded-2xl bg-sky-50 p-3"><div className="mb-2 text-xs font-black text-slate-800">图片裁切</div><div className="grid grid-cols-2 gap-2">{numberField('焦点 X', selectedElement.imageCropX ?? 50, (imageCropX) => updateElement(selectedElement.id, { imageCropX }), 0, 100)}{numberField('焦点 Y', selectedElement.imageCropY ?? 50, (imageCropY) => updateElement(selectedElement.id, { imageCropY }), 0, 100)}<label className="col-span-2 grid gap-1 text-[10px] font-bold text-[var(--vr-text-muted)]"><span>裁切缩放 {Math.round((selectedElement.imageCropScale ?? 1) * 100)}%</span><input type="range" min="1" max="3" step="0.01" value={selectedElement.imageCropScale ?? 1} onChange={(event) => updateElement(selectedElement.id, { imageCropScale: Number(event.target.value) })} className="accent-indigo-600" /></label></div></section>}<div className="hidden space-y-4">
