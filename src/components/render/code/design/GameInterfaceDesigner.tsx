...
export function GameInterfaceDesigner({
      ...
      setScale(
        Math.max(
          ...
          Math.min(
            ...
            (element.clientHeight - 48) / value.height,
            1,
          ),
        ),
      );
  ...
  const previewScaleLabel = `${Math.round(scale * 100)}%`;

  return (
    <main
      className="min-h-0 grid flex-1 bg-[var(--vr-bg)]"
      style={{ gridTemplateColumns: 'minmax(0, 1fr) minmax(300px, 360px)' }}
      data-game-interface-designer={target}
    >
      <section className="min-h-0 min-w-0 bg-[var(--vr-surface-soft)] flex flex-col">
        <div className="grid h-12 grid-cols-[1fr_auto] items-center gap-3 border-b border-[var(--vr-border)] px-4">
          <div className="flex min-w-0 items-center gap-2 text-xs font-black tracking-wide text-[var(--vr-text-soft)]">
            <Play className="h-4 w-4 text-[var(--vr-accent)]" />
            <span className="truncate">{t('界面预览', 'Interface preview', '画面プレビュー')}</span>
          </div>
          <div className="flex items-center gap-1">
            <span className="mr-2 text-[10px] text-[var(--vr-text-muted)]">
              {value.width} × {value.height} · {previewScaleLabel}
            </span>
            <button
              type="button"
              aria-label={t('撤销', 'Undo', '元に戻す')}
              title={t('撤销', 'Undo', '元に戻す')}
              ...
              className="grid h-8 w-8 place-items-center rounded-lg text-[var(--vr-text-muted)] transition-colors hover:bg-[var(--vr-surface)] hover:text-[var(--vr-accent)] disabled:opacity-30"
            >
        ...
        <div className="min-h-0 flex-1 p-4 xl:p-5" ref={viewport}>
      ...
      <aside className="relative min-h-0 border-l border-[var(--vr-border)] bg-[var(--vr-surface)] backdrop-blur-xl flex flex-col">
        <div className="h-12 px-4 border-b border-[var(--vr-border)] flex items-center justify-between gap-3 text-xs font-black uppercase tracking-wide text-[var(--vr-text-soft)]">
          <div className="flex min-w-0 items-center gap-2">
            ...
            <span className="truncate">{t('界面设计', 'Interface design', '画面デザイン')}</span>
          </div>
          <span className="shrink-0 rounded-md bg-[var(--vr-surface-soft)] px-2 py-1 text-[10px] font-black normal-case text-[var(--vr-text-muted)]">
            {GAME_TARGET_NAMES[target]}
          </span>
        </div>

        <div className="video-render-scroll web-workspace-inspector-scroll min-h-0 flex-1 overflow-y-auto p-4">
          <div className="space-y-3">
            <div className="grid h-9 grid-cols-4 overflow-hidden rounded-lg bg-[var(--vr-surface-soft)]">
              {(Object.keys(labels) as Selection[]).map((key) => {
                ...
                return (
                  <button
                    key={key}
                    type="button"
                    className={`flex h-9 min-w-0 items-center justify-center border-0 px-1 text-[10px] font-black transition-colors ${active ? 'bg-[var(--vr-accent)] text-white' : 'text-[var(--vr-text-soft)] hover:bg-white/5 hover:text-[var(--vr-text)]'}`}
                    aria-pressed={active}
            ...
            <div className="rounded-lg bg-[var(--vr-surface-soft)] p-2">
              <button
                type="button"
                className="flex h-9 w-full items-center gap-2 rounded-lg px-2 text-left text-[11px] font-black text-[var(--vr-text-soft)] transition-colors hover:bg-white/5 hover:text-[var(--vr-text)]"
                aria-expanded={templatesOpen}
                onClick={() => setTemplatesOpen(!templatesOpen)}
              >
                <LayoutTemplate className="h-3.5 w-3.5 text-[var(--vr-accent)]" />
                <span className="flex-1">{t('模板库', 'Templates', 'テンプレート')}</span>
                <span className="text-[10px] text-[var(--vr-text-muted)]">
                  {templatesOpen ? '−' : '+'}
                </span>
              </button>
              {templatesOpen && (
                <div className="mt-2 grid grid-cols-2 gap-2">
                  {experienceThemes.map((template) => (
                    <button
                      key={template.id}
                      type="button"
                      className="group overflow-hidden rounded-xl border border-[var(--vr-border)] bg-[var(--vr-surface)] text-left transition-colors hover:border-[var(--vr-accent)] hover:bg-white/5"
                      onClick={() => {
                        update({
                          ...
                          choiceAppearance: {
                            ...
                            fills: [
                              { ...newPaint(), id: 'template-choice', color: template.accent },
                            ],
                          },
                        });
                    ...
                    >
                      ...
                      />
                      <span className="block truncate p-2 text-[10px] font-black">
                        {template.name}
                      </span>
                    </button>
                  ))}
            ...
            <p className="px-1 text-[10px] leading-4 text-[var(--vr-text-muted)]">
              {t(
                '设置仅应用于当前游戏引擎。',
            ...
            />
            {selection === 'dialogue' && cornersOpen && (
              <FloatingPopover
                language={language}
                popoverKey="corners"
