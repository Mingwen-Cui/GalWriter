...
export function normalizeGameInterface(
  input?: Partial<GameInterfaceSettings>,
  ...
  for (const [key, [min, max]] of Object.entries(ranges)) {
    ...
    if (Number.isFinite(n))
      Object.assign(value, { [key]: Math.max(min, Math.min(max, Math.round(n))) });
  }
...
export function resolveGameInterface(
  profiles: GameInterfaceProfiles | undefined,
