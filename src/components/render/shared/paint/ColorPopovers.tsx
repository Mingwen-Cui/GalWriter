...
const toneClass: Record<PopoverTone, string> = {
  fill: 'property-paint-popover',
  stroke: 'property-paint-popover',
  shadow: 'property-paint-popover',
};
...
export function GradientPopover({
  ...
  angle,
  gradientType = 'linear',
  allowedTypes = ['linear', 'radial', 'angular', 'diamond'],
  stops,
  onAngleChange,
  ...
  allowedTypes?: RenderGradientType[];
    ...
    <div className={`rounded-xl border p-3 shadow-xl ${toneClass[tone]}`}>
      ...
      <div ref={gradientBarRef} className="relative mb-8 h-8 rounded-lg border border-white/80">
        <div
          ...
          style={{ background: gradientPreview('linear', 90, orderedStops) }}
        />
        {orderedStops.map((stop) => (
          <button
            ...
            title={`${text.gradientTitle} ${stop.position}%`}
            aria-label={`${text.gradientTitle} ${stop.position}%`}
            onKeyDown={(event) => {
