...
export function RenderObjectSettingsSection({
  ...
  return (
    <div className="space-y-3">
      {!hideSelectionBar && surface !== 'video' && canvasSettings?.layoutMode === 'classic' && onCanvasSettingsChange && (
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-4 gap-2 rounded-2xl border border-[var(--vr-border)] bg-[var(--vr-surface)] p-3">
            {(['dialogBox', 'title', 'body', 'nameplate'] as RenderEditableObjectKind[]).map((kind) => <button key={kind} type="button" aria-pressed={currentSelection === kind} data-render-selection={kind} onPointerDown={(event) => event.stopPropagation()} onClick={() => selectObject(kind)} className={`h-9 min-w-0 truncate rounded-lg px-2 text-left text-xs font-bold transition-colors ${currentSelection === kind ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>{labels[kind]}</button>)}
          </div>
        </div>
      )}
