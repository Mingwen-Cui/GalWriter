...
export function RenderObjectSettingsSection({
  ...
  useEffect(() => {
    if (renderStyle.selectedRenderObject && !selection)
      setActiveSelection(renderStyle.selectedRenderObject);
    ...
    <div className="space-y-3">
        ...
        onCanvasSettingsChange && (
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2">
              {(['scene', 'background'] as const).map((kind) => (
                <button
                  key={kind}
                  type="button"
                  ...
                  className={`h-9 rounded-lg px-2 text-left text-xs font-bold transition-colors ${currentSelection === kind ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}
                >
                  {labels[kind]}
                </button>
              ))}
      ...
      onCanvasSettingsChange ? (
        <SceneCanvasInspector
          ...
          mode={currentSelection === 'scene' ? 'position' : 'background'}
        />
      ) : (
        <RenderObjectInspector
          language={language}
          renderStyle={inspectorStyle}
          updateRenderStyle={updateRenderStyle}
          surface={surface}
          showDescriptions={showDescriptions}
          hideObjectSelector={
            hideSelectionBar || surface === 'video' || canvasSettings?.layoutMode === 'classic'
          }
