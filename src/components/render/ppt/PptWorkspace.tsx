...
import { getNameplateCssBackground } from '../video/shared/nameplateRenderer';
...
function SlideCanvas({ selectedId, isChoiceSlide = false, scene, projectName, webSettings, renderStyle, colors, animations, transition, selected, previewing, onSelect, onChoose }: { selectedId: string; isChoiceSlide?: boolean; scene?: Scene; projectName: string; webSettings: WebExportSettings; renderStyle: RenderStyle; colors: ReturnType<typeof pptSceneColors>; animations: PptObjectAnimation[]; transition: PptSlideTransition; selected: Selection | null; previewing: boolean; onSelect: (selection: Selection) => void; onChoose?: (targetId: string) => void }) { const transitionStyle = transition.effect === 'none' ? undefined : { animationDuration: `${transition.durationMs}ms` }; return <div className={`ppt-slide-canvas ppt-transition-${transition.effect} relative aspect-video w-full overflow-hidden rounded-xl border border-white/15 bg-slate-950 shadow-2xl`} style={{ backgroundColor: selectedId === 'cover' ? webSettings.startMenuBackgroundColor : colors.background, ...transitionStyle }}>{selectedId === 'cover' ? <CoverPreview projectName={projectName} selected={selected} animations={animations} previewing={previewing} onSelect={onSelect} /> : scene ? isChoiceSlide ? <ChoicePreview scene={scene} colors={colors} onChoose={onChoose} /> : <ScenePreview scene={scene} renderStyle={renderStyle} colors={colors} selected={selected} animations={animations} previewing={previewing} onSelect={onSelect} /> : null}</div>; }
