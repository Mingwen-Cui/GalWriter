...
function SlideThumbnail({ slide, scenes, projectName, webSettings, renderStyle, colors, animations, transition }: ThumbnailProps) { const scene = slide.id === 'cover' ? undefined : scenes.find((item) => item.id === slide.id); return <div className="pointer-events-none aspect-video overflow-hidden rounded bg-slate-950"><div style={{ width: '714.2857%', transform: 'scale(.14)', transformOrigin: 'top left' }}><SlideCanvas selectedId={slide.id} scene={scene} projectName={projectName} webSettings={webSettings} renderStyle={renderStyle} colors={colors} animations={animations} transition={transition} selected={null} previewing={false} onSelect={() => undefined} /></div></div>; }
