...
export function PptSlideBackgroundInspector({
  ...
  const canvas = normalizeSharedCanvasSettings({
    canvasWidth: standard ? 1440 : 1920,
    canvasHeight: 1080,
    canvasRatioWidth: standard ? 4 : 16,
    canvasRatioHeight: standard ? 3 : 9,
  });
  return (
    <div className="property-inspector space-y-1">
      <CanvasSettingsSection
        ...
        onChange={(patch) => {
          const layout = patch.canvasRatioWidth === 4 ? 'LAYOUT_STANDARD' : 'LAYOUT_WIDE';
      ...
      {pendingLayout && (
        <PptLayoutChangeDialog
          copy={getPptCopy(language)}
          onCancel={() => setPendingLayout(null)}
          onChoose={(layoutContentMode) => {
            onUpdatePptSettings({ layout: pendingLayout, layoutContentMode });
