...
export function PptExportRulesRibbon({
  ...
  const applyLayout = (layoutContentMode: NonNullable<PptExportSettings['layoutContentMode']>) => {
    ...
    updatePptSettings({ layout: pendingLayout, layoutContentMode });
      ...
      <section className="ppt-export-rules" aria-label={copy.exportRules}>
        <label className="ppt-export-rules-field">
          ...
          <select
            ...
            onChange={(event) => {
              const nextLayout = event.target.value as PptExportSettings['layout'];
              if (nextLayout !== pptSettings.layout) setPendingLayout(nextLayout);
            }}
