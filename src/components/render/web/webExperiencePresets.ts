    ...
    labels.language === 'ja'
      ...
      : labels.language === 'en'
        ? {
            night: [
              'A Quiet Night',
              'The story begins beyond the silence.',
              'Begin story',
              'View archive',
              'Preferences',
            ],
...
const makeStartElements = (
  ...
  const continueLabel = formatWebText(
    labels.language,
    'componentsrenderwebwebExperiencePresetsText152',
  );
...
const makeArchiveElements = (
  ...
  buttonElement({
    id: 'archive-slot-continue',
    role: 'slotContinue',
    text: formatWebText(labels.language, 'componentsrenderwebwebExperiencePresetsText383'),
    x: 56,
    y: 50,
    width: 12,
    height: 5,
  ...
  buttonElement({
    id: 'archive-slot-delete',
    role: 'slotDelete',
    text: formatWebText(labels.language, 'componentsrenderwebwebExperiencePresetsText396'),
    x: 69,
    y: 50,
    width: 7,
    height: 5,
...
const makeSettingsElements = (
  ...
  buttonElement({
    ...
    role: 'animationSpeed',
    text: labels.settingsAnimationSpeed,
    x: 28,
    y: 58,
    width: 44,
    height: 6,
...
const presetElementLayout = (
  ...
  if (surface === 'start' && theme === 'paper') {
    ...
    if (role === 'new' || role === 'save' || role === 'settings')
      return { x: 13, width: 28, height: 8 };
  }
  if (surface === 'archive') {
    if (theme === 'campus') {
  ...
  if (surface === 'settings') {
    if (theme === 'campus') {
      ...
      if (role === 'back') return { x: 74, y: 14, width: 16, height: 7 };
      if (role === 'auto' || role === 'speed' || role === 'controls')
        return { x: 10, width: 76, height: 8 };
    }
    if (theme === 'minimal') {
      if (role === 'title') return { x: 10, y: 12, width: 50, height: 8 };
      if (role === 'back') return { x: 82, y: 12, width: 9, height: 6 };
      if (role === 'auto' || role === 'speed' || role === 'controls')
        return { x: 10, width: 80, height: 7 };
    }
    if (theme === 'paper') {
      if (role === 'title') return { x: 14, y: 13, width: 48, height: 10 };
      if (role === 'back') return { x: 74, y: 14, width: 15, height: 7 };
      if (role === 'auto' || role === 'speed' || role === 'controls')
        return { x: 14, width: 54, height: 8 };
    }
    if (theme === 'neon') {
      if (role === 'title') return { x: 10, y: 10, width: 56, height: 9 };
      if (role === 'back') return { x: 76, y: 10, width: 14, height: 7 };
      if (role === 'auto' || role === 'speed' || role === 'controls')
        return { x: 18, width: 64, height: 8 };
    }
  }
  return {};
};
...
const stylePresetElements = (
  ...
  const palette = {
    night: {
      ...
      radius: 18,
      font: 'ui-serif, "Noto Serif SC", "Songti SC", serif',
    },
    campus: {
      text: '#0f3b4a',
      soft: '#155e75',
    ...
    minimal: {
      text: '#e2e8f0',
      soft: '#94a3b8',
      card: 'rgba(15,23,42,0.44)',
      border: 'rgba(148,163,184,0.42)',
      radius: 4,
      font: 'ui-monospace, "Cascadia Mono", "Noto Sans Mono CJK SC", monospace',
    },
    paper: {
      text: '#451a03',
      soft: '#78350f',
  ...
  return elements.map((element) => {
    ...
    if (theme === 'paper') {
      return {
        ...
        ...base,
        ...layout,
        backgroundColor: isPrimary ? '#a16207' : palette.card,
        borderWidth: 1.2,
      };
    }
    if (theme === 'campus') {
      return {
        ...
        ...base,
        ...layout,
        backgroundColor: isPrimary ? '#14b8a6' : palette.card,
        borderWidth: 1,
      };
    }
    return {
      ...
      ...base,
      ...layout,
      backgroundColor: isPrimary ? element.backgroundColor : palette.card,
      borderWidth: theme === 'minimal' ? 1 : undefined,
    };
  });
};
...
const makePresetSurfaceSettings = (theme: PresetVisualTheme): Partial<WebExportSettings> => {
  type SurfaceTheme = {
    ...
    archive: [string, string, number];
    settings: [string, string, number];
    dialogue: [string, string, number];
  ...
  const allThemes: Record<PresetVisualTheme, SurfaceTheme> = {
    night: {
      start: ['#020617', '#312e81', 145],
      archive: ['#020617', '#0c4a6e', 160],
      settings: ['#0f172a', '#312e81', 125],
      dialogue: ['#020617', '#0f172a', 180],
      layoutMode: 'immersive' as const,
      choicesPosition: 'center' as const,
      blurBackground: true,
      skipSingleChoicePopup: true,
    },
    campus: {
      start: ['#e0f2fe', '#99f6e4', 160],
      archive: ['#ecfeff', '#a7f3d0', 135],
      settings: ['#f0fdfa', '#bae6fd', 110],
      dialogue: ['#cffafe', '#e0f2fe', 145],
      layoutMode: 'classic' as const,
      choicesPosition: 'belowText' as const,
      blurBackground: false,
      skipSingleChoicePopup: false,
    },
    minimal: {
      start: ['#0f172a', '#111827', 180],
      archive: ['#111827', '#1e293b', 145],
      settings: ['#0f172a', '#334155', 125],
      dialogue: ['#020617', '#111827', 180],
      layoutMode: 'classic' as const,
    ...
    paper: {
      start: ['#fef3c7', '#fed7aa', 145],
      archive: ['#fffbeb', '#fde68a', 115],
      settings: ['#fefce8', '#fed7aa', 150],
      dialogue: ['#fef3c7', '#ffedd5', 180],
      layoutMode: 'classic' as const,
      choicesPosition: 'belowText' as const,
      blurBackground: false,
      skipSingleChoicePopup: false,
    ...
    neon: {
      start: ['#020617', '#0e7490', 120],
      archive: ['#020617', '#3b0764', 135],
      settings: ['#020617', '#164e63', 105],
      dialogue: ['#020617', '#0f172a', 180],
      layoutMode: 'immersive' as const,
      choicesPosition: 'center' as const,
      blurBackground: true,
  ...
  return {
    ...
    startMenuBackgroundGradientStart: startColor,
    startMenuBackgroundGradientEnd: startEnd,
    startMenuBackgroundGradientAngle: startAngle,
    archiveBackgroundType: 'gradient',
    archiveBackgroundGradientStart: archiveColor,
    archiveBackgroundGradientEnd: archiveEnd,
    archiveBackgroundGradientAngle: archiveAngle,
    settingsBackgroundType: 'gradient',
    settingsBackgroundGradientStart: settingsColor,
    settingsBackgroundGradientEnd: settingsEnd,
    settingsBackgroundGradientAngle: settingsAngle,
    dialogueBackgroundType: 'gradient',
    dialogueBackgroundGradientStart: dialogueColor,
    dialogueBackgroundGradientEnd: dialogueEnd,
    dialogueBackgroundGradientAngle: dialogueAngle,
    layoutMode: themes.layoutMode,
    choicesPosition: themes.choicesPosition,
    blurBackground: themes.blurBackground,
    skipSingleChoicePopup: themes.skipSingleChoicePopup,
  };
};
...
export const buildWebExperiencePresets = (labels: PresetLabels): WebExperiencePreset[] => [
  {
    ...
    settings: {
      ...
      startMenuElements: stylePresetElements(
        makeStartElements(makePresetLabels(labels, 'night'), '#0ea5e9', '#ffffff', 'center'),
        'night',
        'start',
      ),
      archivePageElements: stylePresetElements(
        makeArchiveElements(labels, '#0ea5e9', '#ffffff'),
        'night',
        'archive',
      ),
      settingsPageElements: stylePresetElements(
        makeSettingsElements(labels, '#0ea5e9', '#ffffff'),
        'night',
        'settings',
      ),
    },
    renderStyle: {
      ...
      dialogBackgroundType: 'gradient',
      dialogGradientStartColor: '#0f172a',
      dialogGradientColor: '#1e1b4b',
      dialogGradientAngle: 125,
      nameplateInside: true,
      nameplateRadius: 14,
      titleAnimation: 'fade',
  ...
  },
  {
    ...
    settings: {
      ...
      startMenuElements: stylePresetElements(
        makeStartElements(makePresetLabels(labels, 'campus'), '#14b8a6', '#ffffff', 'left'),
        'campus',
        'start',
      ),
      archivePageElements: stylePresetElements(
        makeArchiveElements(labels, '#14b8a6', '#ffffff'),
        'campus',
        'archive',
      ),
      settingsPageElements: stylePresetElements(
        makeSettingsElements(labels, '#14b8a6', '#ffffff'),
        'campus',
        'settings',
      ),
    },
    renderStyle: {
      ...
      dialogBackgroundType: 'gradient',
      dialogGradientStartColor: '#ecfeff',
      dialogGradientColor: '#cffafe',
      dialogGradientAngle: 145,
  ...
  },
  {
    ...
    settings: {
      ...
      startMenuElements: stylePresetElements(
        makeStartElements(makePresetLabels(labels, 'minimal'), '#475569', '#ffffff', 'quiet'),
        'minimal',
        'start',
      ),
      archivePageElements: stylePresetElements(
        makeArchiveElements(labels, '#475569', '#ffffff'),
        'minimal',
        'archive',
      ),
      settingsPageElements: stylePresetElements(
        makeSettingsElements(labels, '#475569', '#ffffff'),
        'minimal',
        'settings',
      ),
    },
  ...
  },
  {
    id: 'paper-story',
    name: '纸页小说',
    description: '暖色纸张、衬线排版、适合叙事与阅读。',
    accent: '#a16207',
    choiceColor: '#a16207',
    choiceTextColor: '#ffffff',
    settings: {
      ...makePresetSurfaceSettings('paper'),
      showStartMenu: true,
      startMenuTemplate: 'minimal',
      startMenuElements: stylePresetElements(
        makeStartElements(makePresetLabels(labels, 'campus'), '#a16207', '#ffffff', 'left'),
        'paper',
        'start',
      ),
      archivePageElements: stylePresetElements(
        makeArchiveElements(labels, '#a16207', '#ffffff'),
        'paper',
        'archive',
      ),
      settingsPageElements: stylePresetElements(
        makeSettingsElements(labels, '#a16207', '#ffffff'),
        'paper',
        'settings',
      ),
    },
    renderStyle: {
      ...
      titleColor: '#451a03',
      bodyColor: '#78350f',
      nameplateColor: '#a16207',
      dialogBackgroundType: 'gradient',
      dialogGradientStartColor: '#fffbeb',
      dialogGradientColor: '#fed7aa',
  ...
  },
  {
    id: 'neon-interface',
    name: '霓虹界面',
    description: '深色霓虹、锐利边角、适合科幻和赛博作品。',
    accent: '#22d3ee',
    choiceColor: '#0891b2',
    choiceTextColor: '#ecfeff',
    settings: {
      ...makePresetSurfaceSettings('neon'),
      showStartMenu: true,
      startMenuTemplate: 'cinematic',
      startMenuElements: stylePresetElements(
        makeStartElements(makePresetLabels(labels, 'night'), '#0891b2', '#ecfeff', 'center'),
        'neon',
        'start',
      ),
      archivePageElements: stylePresetElements(
        makeArchiveElements(labels, '#0891b2', '#ecfeff'),
        'neon',
        'archive',
      ),
      settingsPageElements: stylePresetElements(
        makeSettingsElements(labels, '#0891b2', '#ecfeff'),
        'neon',
        'settings',
      ),
    },
    renderStyle: {
      ...
      titleColor: '#67e8f9',
      bodyColor: '#cffafe',
      nameplateColor: '#22d3ee',
      dialogBackgroundType: 'gradient',
      dialogGradientStartColor: '#020617',
      dialogGradientColor: '#164e63',
      dialogGradientAngle: 110,
      nameplateInside: false,
      nameplateRadius: 2,
      titleAnimation: 'slideUp',
      bodyAnimation: 'typewriter',
...
export const buildDefaultWebExperiencePreset = (
  ...
  buildWebExperiencePresets({
    language,
    title,
    subtitle: formatWebText(language, 'componentsrenderwebwebExperiencePresetsText717'),
    save: formatWebText(language, 'componentsrenderwebwebExperiencePresetsText718'),
    newGame: formatWebText(language, 'componentsrenderwebwebExperiencePresetsText719'),
    settings: formatWebText(language, 'componentsrenderwebwebExperiencePresetsText720'),
    archiveTitle: formatWebText(language, 'componentsrenderwebwebExperiencePresetsText721'),
    archiveBack: formatWebText(language, 'componentsrenderwebwebExperiencePresetsText722'),
    archiveSlot: formatWebText(language, 'componentsrenderwebwebExperiencePresetsText723'),
    archiveNew: formatWebText(language, 'componentsrenderwebwebExperiencePresetsText724'),
    settingsTitle: formatWebText(language, 'componentsrenderwebwebExperiencePresetsText725'),
    settingsBack: getWebSettingsCopy(language).backToMainMenu,
    settingsAuto: formatWebText(language, 'componentsrenderwebwebExperiencePresetsText727'),
    settingsSpeed: formatWebText(language, 'componentsrenderwebwebExperiencePresetsText728'),
    settingsControls: formatWebText(language, 'componentsrenderwebwebExperiencePresetsText729'),
