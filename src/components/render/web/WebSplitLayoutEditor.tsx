...
export function WebSplitLayoutEditor({
  ...
  const select = (next: SplitEditorSelection) => {
    ...
    onRenderStyleChange(
      ...
      next === 'scene' || next === 'background' ? undefined : next,
    );
  };
  ...
  const beginMove = (event: React.PointerEvent, kind: SplitEditorSelection) => {
    ...
    if (kind === 'scene') {
      ...
      const move = (moveEvent: PointerEvent) => {
        ...
        onCanvasSettingsChange({
          sceneOffsetX: clamp(
            initialX + ((moveEvent.clientX - startX) / Math.max(1, rootRect.width)) * 200,
            -100,
            100,
          ),
          sceneOffsetY: clamp(
            initialY + ((moveEvent.clientY - startY) / Math.max(1, rootRect.height)) * 200,
            -100,
            100,
          ),
        });
    ...
    const move = (moveEvent: PointerEvent) => {
      const next = updateRenderObject(renderStyle, kind, {
        x: Math.round(object.x + moveEvent.clientX - startX),
        y: Math.round(object.y + moveEvent.clientY - startY),
      });
  ...
  const beginResize = (
    ...
    kind: Exclude<SplitEditorSelection, 'background'>,
    ...
    if (kind === 'scene') {
      ...
      bindWindowDrag((moveEvent) => {
        ...
        const nextX = moveEvent.shiftKey
          ? changesWidth
            ? clamp(initialX + deltaX, 25, 400)
            : initialX
          : (() => {
              const factor =
                changesHeight &&
                (!changesWidth ||
                  Math.abs(deltaY / Math.max(initialY, 1)) >
                    Math.abs(deltaX / Math.max(initialX, 1)))
        ...
        onCanvasSettingsChange({
          sceneScaleX: nextX,
          sceneScaleY: nextY,
          sceneScale: Math.round((nextX + nextY) / 2),
        });
      });
      return;
    }
    ...
    bindWindowDrag((moveEvent) => {
      ...
      const width = changesWidth
        ? Math.max(
            kind === 'dialogBox' ? 35 : 8,
            object.width + (dx * object.width) / Math.max(1, box?.width || 1),
          )
        : object.width;
      const height = changesHeight
        ? Math.max(
            kind === 'dialogBox' ? 16 : 8,
            object.height + (dy * object.height) / Math.max(1, box?.height || 1),
          )
        : object.height;
      const next = updateRenderObject(renderStyle, kind, {
    ...
    <div className="pointer-events-none absolute inset-0 z-[500]" data-split-layout-editor>
      {(['scene', ...kinds] as Array<'scene' | RenderEditableObjectKind>).map((kind) => {
        ...
        return (
          <div
            key={kind}
            className={`absolute ${selected ? 'ring-1 ring-indigo-500' : ''}`}
            style={{
              left: box.x,
              top: box.y,
              width: box.width,
              height: box.height,
              zIndex: kind === 'dialogBox' ? 1 : kind === 'scene' ? 0 : 2,
              pointerEvents: 'none',
            }}
          >
            <button
              ...
              onPointerDown={(event) => beginMove(event, kind)}
              onClick={(event) => {
                event.stopPropagation();
                select(kind);
              }}
