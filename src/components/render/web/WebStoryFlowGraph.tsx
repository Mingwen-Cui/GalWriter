...
export function WebStoryFlowGraph({
  ...
  const bounds = graphBoundsFromPositions(positions, cardWidth, cardHeight);
          ...
          >
            ...
            >
              {graphLinks.map((link) => {
                ...
                const path = segmentLinkPath(from, to, cardWidth, cardHeight, layoutDirection, 28);
                return (
                  <g
                    key={link.id}
            ...
            {segments.map((segment) => {
              ...
              return (
                <button
                  ...
                  className={`absolute overflow-hidden rounded-xl border text-left shadow-lg transition ${inStoryChain ? 'border-indigo-400 shadow-indigo-200/70' : 'border-slate-200 hover:border-indigo-300 hover:shadow-indigo-100/70'} ${active ? 'ring-2 ring-indigo-300/70' : ''}`}
                  style={{
                    left: position.x,
                    top: position.y,
                    width: cardWidth,
