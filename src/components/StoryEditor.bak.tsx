...
import {
  ...
  Mail, MessageCircle, Copy, Check, FileText, Calculator, Replace, UserCircle2
} from 'lucide-react';
...
import { exportPaths, downloadText } from '../lib/export';
...
export function StoryEditor() {
  ...
  const [forceFlowchartMode, setForceFlowchartMode] = useState(false);
  ...
  }, [nodes, edges, canvasBg, edgeStyle, customApiKey, pasteAsPlainText, showNodeActions, showStats, presetColors, forceFlowchartMode, showTitles, generateLength, aiProvider, deepseekApiKey, openaiApiKey, thinkingMode, aiPrompts, aiButtonsConfig, scrollMode, showMiniMap, showControls, toolbarLayout, selectionMenuLayout, language, theme, playTestDarkMode, playTestChoicesColumns, playTestVideoAutoPlay, playTestLayoutMode, playTestInteractionMode, playTestTypewriterSpeed, playTestChoiceDelay, playTestChoicesPosition, playTestBlurBackground, playTestBlurText, playTestSkipSingleChoicePopup, playTestDimBackground]);
    ...
    try {
      ...
      if (data.nodes && data.edges) {
        ...
        if (data.settings) {
          ...
          if (data.settings.forceFlowchartMode !== undefined) setForceFlowchartMode(data.settings.forceFlowchartMode);
  ...
  const handleAIGenerate = useCallback(async (nodeId: string, action: 'continue' | 'creative' | 'rewrite' | 'interpolate' | 'scene_only' | 'dialogue_only') => {
    ...
    try {
      ...
      // Inject character context into contextText
      const finalContextText = charContext ? `${charContext}\n\n${contextText}` : contextText;
  ...
  }, [nodes, edges, aiProvider, deepseekApiKey, customApiKey, openaiApiKey, thinkingMode, buildContextText, handleUpdateNode, buildPrompt]);
  ...
  const nodesWithCallbacks = useMemo(() => {
    return nodes.map(n => {
      const outEdgeCount = edges.filter(e => e.source === n.id).length;
      let effectiveShape = n.data.shape;

      if (forceFlowchartMode) {
        if (n.data.isRoot || outEdgeCount === 0) {
          effectiveShape = 'rounded-rectangle';
        } else if (outEdgeCount > 1) {
    ...
    // 防止闭包过期导致这些引用读到旧值
  }, [nodes, edges, forceFlowchartMode, showTitles, aiLoadingNodeId, handleUpdateNode, handleAddConnectedNode, handleDeleteNode, handleAIButtonClick, handleAIAnalyze, toggleStorylineHighlight, highlightedPath, pasteAsPlainText, showNodeActions, language, theme]);
  ...
  }, [edges, edgeStyle, handleEdgeDelete, highlightedPath]);
    ...
    `}</style>
        ...
        >
          ...
          {!toolbarCollapsed && (
            <div className="flex flex-col gap-2 pb-2 animate-in fade-in slide-in-from-top-2 duration-300">
              ...
              {/* <button
                className="p-2.5 rounded-xl flex items-center justify-center hover:bg-slate-100 text-slate-600 transition-colors"
                onClick={() => addNewShape('diamond')}
                title={t.toolDiamond}
              >
                <Diamond fill="currentColor" strokeWidth={0} className="w-6 h-6 opacity-80" />
              </button> */}

              {/* <button
                className="p-2.5 rounded-xl flex items-center justify-center hover:bg-slate-100 text-slate-600 transition-colors"
                onClick={() => addNewShape('rounded-rectangle')}
                title={t.toolRounded}
              >
      ...
      <Suspense fallback={null}>
        <SettingsModal
          ...
          forceFlowchartMode={forceFlowchartMode}
          setForceFlowchartMode={setForceFlowchartMode}
