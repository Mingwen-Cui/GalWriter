...
import { Handle, Position, NodeProps, useStore, useStoreApi, useUpdateNodeInternals } from '@xyflow/react';
import { Calculator, Trash2, ChevronDown, ChevronRight, ArrowUpDown, Plus, X } from 'lucide-react';
...
export function NumberConditionNode({ id, data, selected }: NodeProps) {
  ...
  const greaterStyles = isGreater
    ? { bg: 'bg-emerald-500/10', border: 'border-emerald-500', text: 'text-emerald-400', handle: 'bg-emerald-400 ring-emerald-500/50' }
    : { bg: 'bg-red-500/10', border: 'border-red-500', text: 'text-red-400', handle: 'bg-red-400 ring-red-500/50' };

  const lessEqualStyles = isGreater
    ? { bg: 'bg-emerald-500/10', border: 'border-emerald-500', text: 'text-emerald-400', handle: 'bg-emerald-400 ring-emerald-500/50' }
    : { bg: 'bg-blue-500/10', border: 'border-blue-500', text: 'text-blue-400', handle: 'bg-blue-400 ring-blue-500/50' };

  // NOTE: 使用 getState() 读取当前边连接状态，避免订阅整个 edges 数组
  const hasInput = storeApi.getState().edges.some(e => e.target === id);
  const inputRingClasses = hasInput ? "" : "ring-2 ring-offset-2 ring-offset-[var(--card-bg)] ring-amber-500/50";
  const handleClasses = "w-3 h-3 bg-amber-300 border-2 border-[var(--card-bg)] rounded-full transition-[transform,background-color] hover:scale-150 hover:bg-amber-500 shadow-sm";
  const inputHandleClasses = `${handleClasses} ${inputRingClasses}`;

  return (
    <div className={`w-[250px] bg-[var(--card-bg)] rounded-xl shadow-lg border-2 transition-all ${selected ? 'border-amber-500 shadow-amber-500/20' : 'border-[var(--card-border)]'} flex flex-col relative group`}>
        ...
        {!isMinimized && (
          <div className="p-4 flex flex-col gap-4 text-[var(--text-secondary)]">
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-medium text-[var(--text-muted)]">判断阈值 (设定数字)</label>
              <NumberInput
                value={threshold}
            ...
            <div className="flex flex-col gap-2 mt-2">
              {!isReversed ? (
                <>
                  <div className={`flex justify-between items-center p-2 rounded-lg border-2 transition-colors ${greaterStyles.bg} ${greaterStyles.border}`}>
                    <span className={`text-xs font-bold ${greaterStyles.text}`}>累计 &gt; {threshold} (大于)</span>
                  </div>
                  <div className={`flex justify-between items-center p-2 rounded-lg border-2 transition-colors ${lessEqualStyles.bg} ${lessEqualStyles.border}`}>
                    <span className={`text-xs font-bold ${lessEqualStyles.text}`}>累计 ≤ {threshold} (小于等于)</span>
                  </div>
                </>
              ) : (
                <>
            ...
            {ranges.length > 0 && (
              <div className="flex flex-col gap-2">
                {ranges.map((range, index) => (
                  <div key={range.id} className={`flex flex-col gap-2 p-2 rounded-lg border-2 transition-colors ${isInRange(range) ? 'bg-amber-500/10 border-amber-500' : 'bg-[var(--card-bg)] border-[var(--card-border)]'}`}>
                    <div className="flex justify-between items-center">
                      <span className={`text-[10px] font-bold ${isInRange(range) ? 'text-amber-400' : 'text-[var(--text-muted)]'}`}>范围限制 {index + 1}</span>
                      <button onClick={() => removeRange(range.id)} className="text-[var(--text-muted)] hover:text-red-400 transition-colors">
                        <X className="w-3 h-3" />
                      </button>
                    </div>
      ...
      <Handle type="target" position={Position.Left} id="in-left" className={`${inputHandleClasses} -left-1.5 !z-50`} style={{ top: isMinimized ? '20px' : '133px' }} />
      ...
      />
      {!isMinimized && <span className={`absolute right-3 text-[9px] font-bold pointer-events-none transition-colors ${greaterStyles.text} !z-40`} style={{ top: isReversed ? '229px' : '189px' }}>&gt;</span>}
      ...
      />
      {!isMinimized && <span className={`absolute right-3 text-[9px] font-bold pointer-events-none transition-colors ${lessEqualStyles.text} !z-40`} style={{ top: isReversed ? '189px' : '229px' }}>≤</span>}
      ...
      {ranges.map((range, index) => {
        ...
        return (
          <React.Fragment key={range.id}>
            ...
            />
            {!isMinimized && (
              <span className="absolute right-3 text-[9px] font-bold text-[var(--text-muted)] pointer-events-none !z-40" style={{ top: `${topPos - 6}px` }}>
                R{index + 1}
              </span>
            )}
          </React.Fragment>
