...
import { X, ChevronRight, PlayCircle, Loader2, Moon, Sun, Settings, Layout, RotateCcw, Maximize2, Minimize2, Eye, EyeOff } from 'lucide-react';
...
function sliceHtmlByTextLength(html: string, maxTextLength: number): { slicedHtml: string; totalTextLength: number } {
...
export function PlayTestModal({
    ...
    const textHtml = currentNodeId !== 'THE_END' && currentNode ? (currentNode.data.text as string) || '' : '';
    ...
    const renderChoices = (isImmersive: boolean) => {
        ...
        return (
            <div className={gridClass} onClick={(e) => e.stopPropagation()}>
                {outEdges.length > 0 ? outEdges.map((edge, index) => {
                    ...
                    if (targetNode?.type === 'numberConditionNode') {
                        ...
                        const ranges = (targetNode.data.ranges as { id: string, min: number, max: number }[]) || [];
                        let targetHandle = '';
                    ...
                    const isWhite = nodeColor.toLowerCase() === '#ffffff' || nodeColor.toLowerCase() === 'white';
                    ...
                    if (isImmersive) {
                        ...
                        const btnClass = `w-full px-5 py-3.5 rounded-xl border backdrop-blur-md transition-all hover:scale-[1.02] active:scale-[0.98] group flex items-center shadow-lg hover:shadow-xl hover:brightness-110 text-white ${
                            choicesPosition === 'center' ? 'text-center justify-center' : 'text-left justify-between'
                        }`;
                        ...
                        return (
                            ...
                            >
                                <span className="font-bold text-sm md:text-base tracking-wide drop-shadow-sm">{label as string}</span>
                        ...
                        return (
                            ...
                            >
                                <span className="font-bold text-sm md:text-base tracking-wide">{label as string}</span>
                                {choicesPosition !== 'center' && (
                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${isWhite ? (isDarkMode ? 'bg-white/10' : 'bg-slate-200/50') : 'bg-black/10'}`}>
                    ...
                    isImmersive ? (
                        ...
                        >
                            ...
                            {choicesPosition !== 'center' && (
                                <ChevronRight className={`w-6 h-6 ${isDarkMode ? 'text-slate-500' : 'text-slate-400'} animate-pulse`} />
                            )}
                        </button>
                    )
                )}
    ...
    useEffect(() => {
        if (currentNodeId !== 'THE_END' && currentNodeId && !nodes.find(n => n.id === currentNodeId)) {
    ...
    if (!currentNodeId) {
        return (
            <div className={`fixed inset-0 ${isDarkMode ? 'bg-slate-950 text-slate-100' : 'bg-white text-slate-800'} z-[100] flex flex-col items-center justify-center p-6 text-center transition-colors duration-300`}>
                <div className="max-w-md">
                    <h2 className={`text-2xl font-bold mb-3 ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>{t.noScript}</h2>
                    <p className={`${isDarkMode ? 'text-slate-400' : 'text-slate-600'} mb-8`}>{t.createStartNode}</p>
                    <button onClick={onClose} className={`px-8 py-3 ${isDarkMode ? 'bg-sky-600 hover:bg-sky-700 shadow-sky-900/20' : 'bg-indigo-600 hover:bg-indigo-700 shadow-indigo-100'} text-white rounded-xl font-medium shadow-lg transition-all active:scale-95`}>{t.close}</button>
                </div>
            </div>
        );
    }
    ...
    if (currentNode && currentNode.type === 'numberConditionNode') {
        return (
            <div className={`fixed inset-0 ${isDarkMode ? 'bg-slate-950' : 'bg-white'} z-[100] flex items-center justify-center transition-colors duration-300`}>
                <div className="animate-pulse flex flex-col items-center">
                    <Loader2 className={`w-8 h-8 ${isDarkMode ? 'text-sky-400' : 'text-indigo-500'} animate-spin mb-4`} />
                    <span className={`text-sm font-medium ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>计算条件中...</span>
                </div>
            </div>
        );
    }
    ...
    return (
        ...
        >
            ...
            <div 
                ...
                className={`h-14 flex items-center justify-between px-4 md:px-6 shrink-0 z-50 transition-all duration-300 ${
                    ...
                    layoutMode === 'immersive' 
                        ...
                        : (isDarkMode ? 'bg-slate-900 border-b border-white/10 text-white' : 'bg-white border-b border-slate-200 text-slate-800')
                }`}
            >
                <div className="flex items-center gap-2 md:gap-3">
                    <PlayCircle className={`w-5 h-5 ${layoutMode === 'immersive' ? 'text-sky-400' : (isDarkMode ? 'text-sky-400' : 'text-indigo-600')}`} />
                    <span className={`font-bold tracking-wide text-sm md:text-base truncate max-w-[120px] md:max-w-none ${layoutMode === 'immersive' ? 'text-white drop-shadow-sm' : (isDarkMode ? 'text-white' : 'text-slate-800')}`}>{t.playTestTitle}</span>
                </div>
                <div className="flex items-center gap-2 md:gap-4">
                    <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs md:text-sm font-bold transition-all active:scale-95 ${
                            layoutMode === 'immersive'
                                ...
                                : (isDarkMode ? 'bg-sky-500/20 text-sky-300 hover:bg-sky-500/30' : 'bg-indigo-50 text-indigo-600 hover:bg-indigo-100')
                    ...
                    <div className="relative">
                        <button
                            ...
                            className={`p-1.5 md:p-2 rounded-full transition-colors ${
                                showSettings 
                                    ? (layoutMode === 'immersive' ? 'bg-white/25 text-white' : (isDarkMode ? 'bg-white/20' : 'bg-slate-200')) 
                                    : (layoutMode === 'immersive' ? 'bg-white/10 text-white hover:bg-white/20' : (isDarkMode ? 'bg-white/10 text-white' : 'bg-slate-100 text-slate-600 hover:bg-sky-500/10 hover:text-sky-400'))
                            }`}
                        ...
                        {showSettings && (
                            ...
                            >
                                ...
                                <div className="flex items-center justify-between mb-4 px-1 border-t pt-3 border-white/5 order-9">
                                    ...
                                    >
                                        <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${videoAutoPlay ? 'left-6' : 'left-1'}`} />
                                    </button>
                                </div>
                                ...
                                <div className="flex items-center justify-between mb-4 px-1 border-t pt-3 border-white/5 order-8">
                                    ...
                                    <button
                                        ...
                                        className={`w-10 h-5 rounded-full transition-colors relative focus:outline-none ${dimBackground ? (layoutMode === 'immersive' ? 'bg-sky-500' : (isDarkMode ? 'bg-sky-500' : 'bg-indigo-600')) : 'bg-slate-600'}`}
                                    >
                                        <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${dimBackground ? 'left-6' : 'left-1'}`} />
                                    </button>
                                </div>
                                ...
                                <div className={`flex flex-col gap-1 order-2 ${choicesPosition === 'center' ? 'hidden' : ''}`}>
                                    {[1, 2, 3].map(cols => (
                                        <button
                                            key={cols}
                                            onClick={() => { setChoicesColumns(cols); setShowSettings(false); }}
                                            className={`w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition-colors flex items-center justify-between ${
                                                choicesColumns === cols 
                                                    ? (layoutMode === 'immersive' ? 'bg-sky-600 text-white' : (isDarkMode ? 'bg-sky-600 text-white' : 'bg-indigo-600 text-white')) 
                                                    : (layoutMode === 'immersive' ? 'text-slate-300 hover:bg-white/10' : (isDarkMode ? 'text-slate-300 hover:bg-white/10' : 'text-slate-700 hover:bg-slate-100'))
                                            }`}
                                        >
                                            <span>{t[`column${cols}` as keyof typeof t]}</span>
                                            {choicesColumns === cols && <div className="w-1.5 h-1.5 bg-white rounded-full"></div>}
                                        </button>
                                    ))}
                                </div>
                                ...
                                <div className="border-t border-white/10 pt-3 mt-3 space-y-3 order-6">
                                    ...
                                    >
                                        <option value="immediate" className={layoutMode === 'immersive' || isDarkMode ? 'bg-slate-900 text-white' : 'bg-white text-slate-800'}>{language === 'zh' ? '立即显现' : 'Immediate'}</option>
                                        <option value="typewriter" className={layoutMode === 'immersive' || isDarkMode ? 'bg-slate-900 text-white' : 'bg-white text-slate-800'}>{language === 'zh' ? '打字机' : 'Typewriter'}</option>
                                        <option value="timed" className={layoutMode === 'immersive' || isDarkMode ? 'bg-slate-900 text-white' : 'bg-white text-slate-800'}>{language === 'zh' ? '延时展现' : 'Timed Delay'}</option>
                                        <option value="clickToShow" className={layoutMode === 'immersive' || isDarkMode ? 'bg-slate-900 text-white' : 'bg-white text-slate-800'}>{language === 'zh' ? '点击显示' : 'Click to Show'}</option>
                                    </select>

                                    {interactionMode === 'typewriter' && (
                                        <div className="space-y-1">
                                            <div className="flex justify-between items-center px-1">
                                                <span className="text-[10px] opacity-70">{language === 'zh' ? '速度' : 'Speed'}</span>
                                    ...
                                    {interactionMode === 'timed' && (
                                        <div className="space-y-1">
                                            <div className="flex justify-between items-center px-1">
                                                <span className="text-[10px] opacity-70">{language === 'zh' ? '延迟' : 'Delay'}</span>
                                ...
                                <div className="border-t border-white/10 pt-3 mt-3 space-y-3 order-1">
                                    ...
                                    >
                                        <option value="center" className={layoutMode === 'immersive' || isDarkMode ? 'bg-slate-900 text-white' : 'bg-white text-slate-800'}>{language === 'zh' ? '画面中间' : 'Center'}</option>
                                        <option value="aboveText" className={layoutMode === 'immersive' || isDarkMode ? 'bg-slate-900 text-white' : 'bg-white text-slate-800'}>{language === 'zh' ? '文字上方' : 'Above Text'}</option>
                                        <option value="belowText" className={layoutMode === 'immersive' || isDarkMode ? 'bg-slate-900 text-white' : 'bg-white text-slate-800'}>{language === 'zh' ? '文字下方' : 'Below Text'}</option>
                                    </select>
                                </div>
                                ...
                                <div className="border-t border-white/10 pt-3 mt-3 flex items-center justify-between px-1 order-3">
                                    ...
                                    >
                                        <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${blurBackground ? 'left-6' : 'left-1'}`} />
                                    </button>
                                </div>
                                ...
                                {blurBackground && (
                                    <div className="border-t border-white/10 pt-3 mt-3 flex items-center justify-between px-1 animate-in fade-in duration-200 order-4">
                                        ...
                                        >
                                            <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${blurText ? 'left-6' : 'left-1'}`} />
                                        </button>
                                    </div>
                                )}
                                ...
                                {choicesPosition === 'center' && (
                                    <div className="border-t border-white/10 pt-3 mt-3 flex items-center justify-between px-1 animate-in fade-in duration-200 order-5">
                                        ...
                                        >
                                            <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${skipSingleChoicePopup ? 'left-6' : 'left-1'}`} />
                                        </button>
                                    </div>
                                )}
                            </div>
                    ...
                    <div className={`w-px h-4 ${layoutMode === 'immersive' ? 'bg-white/20' : (isDarkMode ? 'bg-white/10' : 'bg-slate-200')}`}></div>
                    ...
                    <button 
                        ...
                        className={`p-1.5 md:p-2 rounded-full transition-colors ${
                            layoutMode === 'immersive'
                                ...
                                : (isDarkMode ? 'bg-white/10 hover:bg-red-500/20 text-white' : 'bg-slate-100 hover:bg-red-50 text-slate-600 hover:text-red-600')
                        }`}
                    >
                        <X className="w-5 h-5" />
                    </button>
            ...
            <div className="flex-1 flex flex-col min-h-0 overflow-hidden relative">
                <div className={`w-full h-full flex flex-col ${isDarkMode ? 'bg-slate-950' : 'bg-slate-50'}`}>
                    ...
                    {currentNodeId === 'THE_END' ? (
                        layoutMode === 'immersive' ? (
                            <div className="flex-1 flex flex-col items-center justify-center p-12 text-center animate-in fade-in duration-500 relative w-full h-full">
                                ...
                                <div className="relative z-10 flex flex-col items-center max-w-md p-8 rounded-2xl bg-black/40 backdrop-blur-md border border-white/10 shadow-2xl">
                                    ...
                                    <h2 className="text-3xl font-bold text-white mb-4 tracking-tight">{t.storyEnd}</h2>
                                    <p className="text-slate-300 mb-8 max-w-sm">{t.branchEnded}</p>
                                    <button
                                        onClick={() => { setHistory([]); navigateToNode(root?.id || null); }}
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center p-12 text-center animate-in fade-in duration-500">
                                <div className={`w-20 h-20 mb-6 rounded-full flex items-center justify-center ${isDarkMode ? 'bg-sky-500/20 text-sky-400' : 'bg-indigo-100 text-indigo-600'}`}>
                                    <PlayCircle className="w-10 h-10" />
                                </div>
                                <h2 className={`text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-slate-800'} mb-4 tracking-tight`}>{t.storyEnd}</h2>
                                <p className={`${isDarkMode ? 'text-slate-400' : 'text-slate-500'} mb-8 max-w-sm`}>{t.branchEnded}</p>
                                <button
                                    onClick={() => { setHistory([]); navigateToNode(root?.id || null); }}
                        ...
                        layoutMode === 'immersive' ? (
                            <div className="flex-1 flex flex-col min-h-0 relative w-full h-full">
                                ...
                                <div className="absolute inset-0 z-0 overflow-hidden w-full h-full select-none pointer-events-none">
                                    {currentNode?.data.imageUrl ? (
                                        ...
                                        <div className={`w-full h-full ${isDarkMode ? 'bg-gradient-to-br from-slate-900 via-sky-950/40 to-slate-950' : 'bg-gradient-to-br from-indigo-50 via-slate-100 to-indigo-100'}`} />
                                    )}
                                    {/* 暗色遮罩层，提升文字对比度 */}
                                    {dimBackground && <div className="absolute inset-0 bg-black/45 z-[1]" />}
                                </div>

                                {/* 2. 悬浮的选项与对话框 */}
                                <div className={`absolute inset-0 flex flex-col justify-end p-4 md:p-6 lg:pb-10 lg:px-48 xl:px-64 pointer-events-none ${
                                        ...
                                        >
                                            ...
                                            {!animationCompleted && (
                                                <div className="absolute right-4 bottom-2 text-[10px] text-white/50 animate-pulse select-none">
                                                    {interactionMode === 'typewriter' && (language === 'zh' ? '点击跳过打字...' : 'Click to skip...')}
                                                    {interactionMode === 'timed' && (language === 'zh' ? `选项将在 ${timeLeft}s 后出现` : `Choices in ${timeLeft}s`)}
                                                    {interactionMode === 'clickToShow' && (language === 'zh' ? '点击文本显示选项...' : 'Click text to show options...')}
                                                </div>
                                            )}
                                ...
                                {/* 选项区域 - 画面中间 */}
                                {choicesPosition === 'center' && animationCompleted && !(skipSingleChoicePopup && outEdges.length <= 1) && (
                                    <div className={`absolute inset-0 z-30 flex items-center justify-center p-6 bg-black/45 pointer-events-none animate-in fade-in duration-300 ${blurBackground ? 'backdrop-blur-[6px]' : 'backdrop-blur-none'}`}>
                            ...
                            <div className="flex-1 flex flex-col min-h-0 relative">
                                {/* 1. Media Area */}
                                {hasMedia && (
                                    <div className={`flex-1 min-h-0 flex items-center justify-center p-2 md:p-6 relative group overflow-hidden ${isDarkMode ? 'bg-slate-950' : 'bg-slate-100'}`}>
                                        ...
                                        <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none transition-all duration-1000">
                                            {currentNode?.data.imageUrl ? (
                                                ...
                                                <div className={`w-full h-full ${isDarkMode ? 'bg-gradient-radial from-sky-500/10' : 'bg-gradient-radial from-indigo-500/5'} to-transparent`} />
                                            )}
                                        </div>
                                        ...
                                        {choicesPosition === 'center' && !isFullscreen && animationCompleted && !(skipSingleChoicePopup && outEdges.length <= 1) && (
                                            <div className={`absolute inset-0 z-30 flex items-center justify-center p-4 bg-black/45 pointer-events-none animate-in fade-in duration-300 ${blurBackground ? 'backdrop-blur-[6px]' : 'backdrop-blur-none'}`}>
                                ...
                                {choicesPosition === 'aboveText' && animationCompleted && (
                                    <div ref={choicesRef} className={`p-4 md:p-6 lg:px-48 xl:px-64 ${isDarkMode ? 'bg-slate-900 border-white/10' : 'bg-slate-100 border-slate-200'} border-b shrink-0 z-40 shadow-sm`}>
                                        {renderChoices(false)}
                                    </div>
                                )}
                                ...
                                <div 
                                    ...
                                    className={`${hasMedia ? 'h-32 md:h-48 shrink-0' : 'flex-1'} overflow-y-auto px-6 py-4 md:px-12 md:py-8 lg:px-48 xl:px-64 ${isDarkMode ? 'bg-slate-950/90' : 'bg-white/90'} backdrop-blur-xl border-t border-white/5 cursor-pointer overflow-hidden transition-all duration-300 relative ${
                                        (choicesPosition === 'center' && animationCompleted && blurBackground)
                                            ? (isFullscreen 
                                                ? (blurText ? 'z-20' : 'z-40') 
                                                : (blurText ? 'z-20 blur-[5px] opacity-80' : 'z-20')
                                              )
                                            : 'z-20'
                                    }`}
                                >
                                    {currentNode?.data.audioUrl && (
                                        <div className={`mb-6 p-2 rounded-xl backdrop-blur-sm ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-slate-900/5 border-slate-200'} border`}>
                                            <audio
                                                key={currentNodeId}
                                                ref={audioRef}
                                                src={currentNode.data.audioUrl as string}
                                ...
                                {choicesPosition === 'belowText' && animationCompleted && (
                                    <div ref={choicesRef} className={`p-4 md:p-6 lg:px-48 xl:px-64 ${isDarkMode ? 'bg-slate-900 border-white/10' : 'bg-slate-100 border-slate-200'} border-t shrink-0 z-40 shadow-[0_-10px_20px_rgba(0,0,0,0.05)]`}>
                                        {renderChoices(false)}
                                    </div>
                                )}

                                {/* 选项区域 - 屏幕的中间（当全屏，或无媒体文件时挂载在整个视口中央） */}
                                        {choicesPosition === 'center' && (isFullscreen || !hasMedia) && animationCompleted && !(skipSingleChoicePopup && outEdges.length <= 1) && (
                                    <div className={`absolute inset-0 z-30 flex items-center justify-center p-6 bg-black/45 pointer-events-none animate-in fade-in duration-300 ${blurBackground ? 'backdrop-blur-[6px]' : 'backdrop-blur-none'}`}>
